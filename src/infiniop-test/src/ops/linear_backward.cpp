...
std::shared_ptr<Test> Test::build(
    ...
    if (tensors.find("grad_x") == tensors.end()
        ...
        || tensors.find("grad_b") == tensors.end()
        ...
        || tensors.find("ans_grad_b") == tensors.end()) {
...
std::shared_ptr<infiniop_test::Result> Test::run(
    ...
    auto grad_b = _attributes->grad_b->to(device, device_id);
    ...
    CHECK_OR(infiniopCreateLinearBackwardDescriptor(handle, &op_desc,
            grad_x->desc(),
            grad_w->desc(),
            grad_b->desc(),
            grad_y->desc(),
    ...
    CHECK_OR(infiniopGetLinearBackwardWorkspaceSize(op_desc, &workspace_size),
             return TEST_FAILED(OP_CREATION_FAILED, "Failed to get workspace size."));
    void *workspace;
    CHECK_OR(infinirtMalloc(&workspace, workspace_size),
             return TEST_FAILED(OP_CREATION_FAILED, "Failed to allocate workspace."));
    CHECK_OR(infiniopLinearBackward(op_desc, workspace, workspace_size,
                    grad_x->data(),
                    grad_w->data(),
                    grad_b->data(),
                    grad_y->data(),
    ...
    try {
        ...
        allClose(grad_b, _attributes->ans_grad_b, _rtol, _atol);
...
std::vector<std::string> Test::tensor_names() {
    return {"grad_x", "grad_w", "grad_b", "grad_y", "x", "w", "ans_grad_x", "ans_grad_w", "ans_grad_b"};
}
...
std::string Test::toString() const {
    ...
    oss << "- grad_b: " << _attributes->grad_b->info() << std::endl;
