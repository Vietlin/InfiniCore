...
namespace infiniop_test::op_and {
...
std::shared_ptr<infiniop_test::Result> Test::run(
    ...
    infiniopAndDescriptor_t op_desc;
    auto a = _attributes->a->to(device, device_id);
    auto b = _attributes->b->to(device, device_id);
    auto c = _attributes->c->to(device, device_id);
    CHECK_OR(infiniopCreateAndDescriptor(handle, &op_desc,
                                         c->desc(),
                                         a->desc(),
                                         b->desc()),
             return TEST_FAILED(OP_CREATION_FAILED, "Failed to create op descriptor."));
    size_t workspace_size;
    CHECK_OR(infiniopGetAndWorkspaceSize(op_desc, &workspace_size),
             return TEST_FAILED(OP_CREATION_FAILED, "Failed to get workspace size."));
    void *workspace;
    CHECK_OR(infinirtMalloc(&workspace, workspace_size),
             return TEST_FAILED(OP_CREATION_FAILED, "Failed to allocate workspace."));
    CHECK_OR(infiniopAnd(op_desc, workspace, workspace_size,
                         c->data(),
                         a->data(),
                         b->data(),
                         nullptr),
