...
INFINIOP_CUDA_KERNEL launchKernel(
    ...
    reduceMaxKernel<Tdata, Tcompute>(
        output,
        input,
        ndim,
        input_shape,
...
infiniStatus_t Descriptor::calculate(
    ...
    #define CALCULATE_REDUCE_MAX(BLOCK_SIZE, TDATA) \
        calculate_reduce_max<BLOCK_SIZE, TDATA>(_info, (TDATA *)output, (const TDATA *)input, stream, workspace)
    #define CALCULATE_REDUCE_MAX_WITH_BLOCK_SIZE(BLOCK_SIZE)          \
    {                                                                 \
        if (_info.dtype == INFINI_DTYPE_F16)                          \
            return CALCULATE_REDUCE_MAX(BLOCK_SIZE, half);            \
        else if (_info.dtype == INFINI_DTYPE_F32)                     \
            return CALCULATE_REDUCE_MAX(BLOCK_SIZE, float);           \
        else if (_info.dtype == INFINI_DTYPE_BF16)                    \
            return CALCULATE_REDUCE_MAX(BLOCK_SIZE, __nv_bfloat16);   \
        else                                                          \
            return INFINI_STATUS_BAD_TENSOR_DTYPE;                    \
    }
    
    if (_opaque->internal->maxThreadsPerBlock() == CUDA_BLOCK_SIZE_1024)
        CALCULATE_REDUCE_MAX_WITH_BLOCK_SIZE(CUDA_BLOCK_SIZE_1024)
    else if (_opaque->internal->maxThreadsPerBlock() == CUDA_BLOCK_SIZE_512)
        CALCULATE_REDUCE_MAX_WITH_BLOCK_SIZE(CUDA_BLOCK_SIZE_512)
    else if (_opaque->internal->maxThreadsPerBlock() == CUDA_BLOCK_SIZE_4096)
        CALCULATE_REDUCE_MAX_WITH_BLOCK_SIZE(CUDA_BLOCK_SIZE_4096)
    else
