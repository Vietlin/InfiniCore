...
//  -------------------------------- start: perform operator on CPU --------------------------------
    ...
    for(size_t n = 0; n < batch_size; n ++)
    {
        ...
        *output_ptr = utils::cast<Tdata>(op::common_cpu::reduce_op::sum(
            input_ptr,
            info.input_shape[info.dim],
            info.input_strides[info.dim]
        ) / float(info.input_shape[info.dim]));
    }
...
#define CALCULATE_REDUCE_MEAN(TDATA) \
    CHECK_STATUS(calculate_reduce_mean<TDATA>(_info, \
...
infiniStatus_t Descriptor::calculate(
    ...
    if (_info.dtype == INFINI_DTYPE_F16) {
        CALCULATE_REDUCE_MEAN(fp16_t);
    } else if (_info.dtype == INFINI_DTYPE_BF16) {
        CALCULATE_REDUCE_MEAN(bf16_t);
    } else if (_info.dtype == INFINI_DTYPE_F32) {
        CALCULATE_REDUCE_MEAN(float);
    } else {
