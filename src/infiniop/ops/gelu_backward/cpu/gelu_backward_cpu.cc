...
infiniStatus_t Descriptor::calculate(
    ...
    #define CALCULATE_GELU_BACKWARD_OP(TDATA) \
        return _device_info->calculate<GeLUBackwardOp, TDATA>(_info, output, inputs, stream)
    switch (_dtype) {
    case INFINI_DTYPE_F16:
        CALCULATE_GELU_BACKWARD_OP(fp16_t);
    case INFINI_DTYPE_F32:
        CALCULATE_GELU_BACKWARD_OP(float);
    case INFINI_DTYPE_BF16:
        CALCULATE_GELU_BACKWARD_OP(bf16_t);
    default:
