...
__C infiniStatus_t infiniopCreateAndDescriptor(
    ...
    case CASE:                                                                  \
        return op::op_and::NAMESPACE::Descriptor::create(                          \
            handle,                                                             \
            reinterpret_cast<op::op_and::NAMESPACE::Descriptor **>(desc_ptr),      \
            c_desc,                                                             \
            {a_desc, b_desc}                                                    \
        )
...
__C infiniStatus_t infiniopGetAndWorkspaceSize(infiniopAndDescriptor_t desc, size_t *size) {
    ...
    case CASE:                                                                                      \
        *size = reinterpret_cast<op::op_and::NAMESPACE::Descriptor *>(desc)->workspaceSize();          \
...
__C infiniStatus_t infiniopAnd(
    ...
    case CASE:                                                                            \
        return reinterpret_cast<const op::op_and::NAMESPACE::Descriptor *>(desc)->calculate( \
            workspace,                                                                    \
            workspace_size,                                                               \
            c,                                                                            \
            {a, b},                                                                       \
...
infiniopDestroyAndDescriptor(infiniopAndDescriptor_t desc) {
    ...
    case CASE:                                                                  \
        delete reinterpret_cast<const op::op_and::NAMESPACE::Descriptor *>(desc);  \
        return INFINI_STATUS_SUCCESS
