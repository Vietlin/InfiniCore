...
def test(
    ...
    print(
        f"Testing and on {InfiniDeviceNames[device]} with input_shape:{input_shape},"
        f"a_stride:{a_stride} b_stride:{b_stride} c_stride:{c_stride},"
        f"dtype:{InfiniDtypeNames[dtype]}"
    )

    torch_a = torch.randint(low=0, high=2, size=input_shape).type(torch.bool)
    if a_stride is not None:
        torch_a = torch_a.as_strided(input_shape, a_stride)
    print(input_shape, torch_a.shape)
    a = TestTensor(
        input_shape,
        torch_a.stride(),
        dtype,
    ...
    torch_b = torch.randint(low=0, high=2, size=input_shape).type(torch.bool)
    if b_stride is not None:
        torch_b = torch_b.as_strided(input_shape, b_stride)
    print(input_shape, torch_b.shape)
    b = TestTensor(
        input_shape,
        torch_b.stride(),
        dtype,
    ...
    if inplace == Inplace.INPLACE_A:
        ...
        torch_c = torch.empty(input_shape, dtype=torch.bool)
        if c_stride is not None:
            torch_c = torch_c.as_strided(input_shape, c_stride)
        c = TestTensor(
            input_shape,
            torch_c.stride(),
            dtype,
    ...
    torch_and(c.torch_tensor(), a.torch_tensor(), b.torch_tensor())
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: torch_and(
...
if __name__ == "__main__":
    ...
    print("\033[92mTest my and passed!\033[0m")
