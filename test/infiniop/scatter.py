...
# _TENSOR_DTYPES = [InfiniDtype.U32]
...
def test(
    ...
    print(
        # TODO:
        f"Testing scatter on {InfiniDeviceNames[device]} with input_shape{input_shape}, index_shape{index_shape}, dim:{dim}"
        f"dtype:{InfiniDtypeNames[dtype]}"
    )
    # torch_dtype = {InfiniDtype.F16: torch.half, InfiniDtype.F32: torch.float, InfiniDtype.BF16: torch.bfloat16}

    # torch_output = None
    # torch_output = torch.zeros(output_shape)
    output = TestTensor(
        output_shape,
        None,
        # torch_output.stride(),
    ...
    torch_index = torch.randint(low=0, high=output.shape[dim], size=(index_shape)).type(torch.int64)
    # torch_index = torch.randint(low=1, high=2, size=(index_shape)).type(torch.int64)
    index = TestTensor(
        index_shape,
        # None,
        torch_index.stride(),
