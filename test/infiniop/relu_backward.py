...
def torch_relu_backward(grad_input: torch.Tensor, input: torch.Tensor, grad_output: torch.Tensor):
    input_clone = input.clone()
    input_clone.requires_grad_(True)
    output = torch.relu(input_clone)
...
def test(
    ...
    print(
        f"Testing relu_backward on {InfiniDeviceNames[device]} with input_shape:{input_shape},"
    ...
    torch_relu_backward(grad_input.torch_tensor(), input.torch_tensor(), grad_output.torch_tensor())
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: torch_relu_backward(
...
if __name__ == "__main__":
    ...
    print("\033[92mTest my relu_backward passed!\033[0m")
