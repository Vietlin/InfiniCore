...
def gather(
    ...
    torch.gather(torch_input, dim, torch_index, out=torch_output)
...
if __name__ == "__main__":
    ...
    _TEST_CASES_ = [
        ...
        ((10, 5, 4), (10, 4, 4), 1, (30, 5, 1), None, [107, 10, 1]),
    ...
    for dtype in _TENSOR_DTYPES_:
        for input_shape, output_shape, dim, input_strides, output_strides, index_strides in _TEST_CASES_:
            input = (np.random.rand(*input_shape) * 100).astype(dtype)
            index = np.random.randint(low=0, high=input_shape[dim], size=output_shape, dtype=np.int64)
