...
class OrTestCase(InfiniopTestCase):
    def __init__(
        self,
        a: np.ndarray,
...
if __name__ == "__main__":
    test_writer = InfiniopTestWriter("op_or.gguf")
    test_cases = []
    # ==============================================================================
    ...
    for shape, stride_a, stride_b, stride_c in _TEST_CASES_:
        a = np.random.randint(low=0, high=2, size=shape).astype(np.bool)
        b = np.random.randint(low=0, high=2, size=shape).astype(np.bool)
