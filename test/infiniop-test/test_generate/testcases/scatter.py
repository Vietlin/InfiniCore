...
if __name__ == "__main__":
    ...
    _TEST_CASES_ = [
        # shape index_shape, output_shape, dim, input_strides
        ((2, 3, 4), (2, 3, 2), (2, 3, 5), 2, (30, 5, 1)),
        ((10, 8, 4), (10, 8, 4), (10, 8, 4), 0, None),
        ((11, 41, 4), (11, 41, 4), (11, 41, 4), 0, (200, 4, 1)),
    ]
    _TENSOR_DTYPES_ = [np.float32, np.float16, np.int64, np.int32]
    for dtype in _TENSOR_DTYPES_:
        for input_shape, index_shape, output_shape, dim, input_strides in _TEST_CASES_:
            input = (np.random.rand(*input_shape) * 100).astype(dtype)
            index = np.empty(index_shape, dtype=np.int64)
            if dim == 0:
                for i in range(input.shape[1]):
                    for j in range(input.shape[2]):
                        arange = list(range(output_shape[dim]))
                        random.shuffle(arange)
                        index[:, i, j] = arange[:index_shape[dim]]
            elif dim == 2:
