...
def linear_backward(
    ...
    return torch_x.grad.detach().numpy(), \
            ...
            torch_b.grad.detach().numpy() if bias_exist else None
...
class LinearBackwardTestCase(InfiniopTestCase):
    ...
    def write_test(self, test_writer: "InfiniopTestWriter"):
        ...
        test_writer.add_array(
            ...
            gguf_strides(*contiguous_gguf_strides(self.grad_x_shape))
        )
        test_writer.add_array(
            test_writer.gguf_key("grad_w.strides"),
            gguf_strides(*contiguous_gguf_strides(self.grad_w_shape))
        )
        test_writer.add_array(
            test_writer.gguf_key("grad_b.strides"),
            gguf_strides(*contiguous_gguf_strides(self.grad_b_shape))
        )
...
if __name__ == "__main__":
    ...
    _TEST_CASES_ = [
        # shape
        (50, 40, True, None, None),
        (50, 40, True, [10], [1]),
        (333, 999, True, [1], [10]),
    ]
    _TENSOR_DTYPES_ = [np.float32, np.float16]
    for dtype in _TENSOR_DTYPES_:
        for in_features, out_features, bias_exist, grad_y_strides, grad_x_strides in _TEST_CASES_:
            ...
            grad_b = np.empty([out_features] if bias_exist else [], dtype=dtype)
            ...
            test_case = LinearBackwardTestCase(
                ...
                grad_w_strides=None,
                grad_b=grad_b,
                grad_b_shape=[out_features] if bias_exist else [],
                grad_b_strides=None,
                grad_y=grad_y,
